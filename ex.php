...
<!--end of php codes -->
    ...
    <div class="col-lg-12">
      ...
      <form method="post" action="">
        <div class="row" style="">
          ...
          <select id="sortclass" onchange="();" style="font-family: sans-serif; font-size: 15px;" class="col-md-4 ">
 ...
 if($chkcateg['category_id']==4){ $ff="SELECT * FROM course inner join user on user.user_id=course.lecture_id inner join class on class.class_id=course.class_id inner join department on department.department_id=class.department_id where class.department_id='$department_id' order by class.section asc";}
        ...
        <?php 
        //Dynamic
        $title='call_class'.$count;  $tt=$title.'();'; $tit='function '.$title.'()'; $divs='sort'.$count; $div='var divnam="sort'.$count.'";'; 

        ?>
<?php echo $tit;?>{
      if(XMLHttpRequestObject){
        XMLHttpRequestObject.open("POST","Details.php");
        XMLHttpRequestObject.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
        XMLHttpRequestObject.onreadystatechange=function(){
        ...
        if(XMLHttpRequestObject.readyState==4 && XMLHttpRequestObject.status==200){
          var datar=XMLHttpRequestObject.responseText;
          <?php echo $div;?>
          var divsee=document.getElementById(divnam);
          divsee.innerHTML=datar;
      ...
      return false;
        ...
        ?>
                          <td style="width: 120px;" id="selc" > 
                              <form method="post" action="">
                                <input type="hidden" value="<?php echo $count;?>" id="sortclass" name="">
                  <input type="button" class="btn btn-info btn-xs" onclick="<?php echo $tt;?>" value="Details">
                </form>
                <?php } ?>
      </form>
                          </td>
