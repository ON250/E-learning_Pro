...
<!--end of php codes -->
	...
	<section>
      ...
      <div class="row">
        <div class="col-md-6">
        <button class="btn btn-info btn-xs col-md-12" data-toggle="modal" data-target="#addnewuser"><i class="fa  fa-plus"> </i> Establish New Exam</button>
      </div>
  ...
  $ff="SELECT *,DATE_FORMAT(exam_date,'%d/%m/%Y') AS exam_d FROM exam inner join course on course.course_id=exam.course_id inner join class on class.class_id=exam.class_id inner join department on department.department_id=exam.department_id inner join test on exam.test=test.ID inner join user on user.user_id=course.lecture_id where exam.department_id='$department_id' and exam.status=1";
        ...
        ?>
                          <td style="width: 130px;"> <small>
                           ...
                           | <a style="text-decoration:none;color:white; width: 60px;"  href="" class="btn btn-info btn-xs"   data-toggle="modal"  <?php  echo 'data-target="#sendexam'.$show['exam_id'].'ers"'; ?> > </span><i class="fa fa-send"></i> Send Exam</span> </a></small></td>
                           </tr>
                          <?php
                      }
                     $count++;
