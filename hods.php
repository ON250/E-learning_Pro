...
<!--php codes -->
  <?php 
   ...
   if($SEEre['existuser']>=1){
      ...
      $enregistrer=mysqli_query($connect,"INSERT INTO user(names,email,phone,status,category_id,pin,password,c_date,department_id) values('$names','$email','$phone','$status','$category','$pin','$ps',NOW(),'$department')");
        if($enregistrer) {
        echo '<script>
                                alert("Head of Department added successfully. Default Password : '.$ps.'")
                          </script>';
      }
      else{
        echo '<script>
    ...
    if(isset($_POST['update']))
    {
      ...
      $edit=mysqli_query($connect,"UPDATE user SET names='$names', email='$email', phone='$phone', status='$status', department_id='$department', category_id='$category' WHERE user_id='$idd'");

        if($edit) {
        echo '<script>
                                alert("Head of Department modified successfully!")
                          </script>';
                        }
        else{
          echo '<script>
...
<!--end of php codes -->
	...
	<div class="container-fluid " style="height: 50px; background: lightgrey"><br>
		<span class="text-right"><?php echo $view['names'];?> | <i style="color:grey;">Head of Department</i> </span>
	</div>
	...
	<section>
		<?php 
			...
			if($chkcateg['category_id']==1){
				?>
		<div class="col-lg-12">
			<button class="btn btn-info btn-xs col-lg-6" data-toggle="modal" data-target="#addnewuser"><i class="fa  fa-plus"> </i>  Add a new Head of Departmet</button>
		</div>
		<?php
			}
		?>
		<div class="col-lg-12">
              <div class="card">
                <div class="card-header">
                  <h2 style="font-family: serif;"><small>List od the Head Of Department</small></h2>
                </div>
                <div class="card-body">
		...
		?>
                      ...
                      <tbody>
                        	...
                        	$sel=mysqli_query($connect,"SELECT * FROM user  inner join department on department.department_id=user.department_id where category_id=4");
...
<!--MODALS -->
 ...
 <div class="modal fade" <?php  echo 'id="delete'.$show['user_id'].'er"';?>   tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
   ...
   <?php } ?>
       ...
       <?php
 //select data from the database
                $select=mysqli_query($connect,"SELECT * FROM user  inner join department on department.department_id=user.department_id where category_id=4");
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <div class="row">
              ...
              <div class="row">
                <select name="category"  hidden="" required=""  class="col-md-9 form-control">
                  <option value="4" class="control">Head of department</option>
                </select>
              </div>
               <div class="row">
                <label class="col-md-3">Status </label>
