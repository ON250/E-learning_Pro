...
<script >
  ...
  function call_lect(){
    ...
    function call_class(){
      if(XMLHttpRequestObject){
        XMLHttpRequestObject.open("POST","sortbyexam.php");
        XMLHttpRequestObject.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
        XMLHttpRequestObject.onreadystatechange=function(){
  ...
  <?php 
   ...
   if($SEEre['exist']>=1){
    ...
    if(isset($_POST['delete']))
    {
      ...
      $edit=mysqli_query($connect,"UPDATE exam SET status='$status',room_id='$room' WHERE exam_id='$idd'");

        if($edit) {
        echo '<script>
                                alert("Exam established successfully!")
                          </script>';
                        }
        else{
          echo '<script>
...
<!--end of php codes -->
 ...
 if($chkcateg['category_id']==5){ 
  ...
  $ff="SELECT *,DATE_FORMAT(exam_date,'%d/%m/%Y') AS exam_d,exam.status as examStatus FROM exam inner join course on course.course_id=exam.course_id inner join class on class.class_id=exam.class_id inner join department on department.department_id=exam.department_id inner join test on exam.test=test.ID inner join user on user.user_id=course.lecture_id where  exam.status!=1 and exam.status!=4";
}
                       ...
                       <tr>
			             ...
			             <?php
                          if($show['examStatus']==2){
        ?>
        <td style="width: 138px;"> <small>
                            <a style="text-decoration:none;color:white;width: 128px; " class="btn btn-info btn-xs" href=""    data-toggle="modal"  <?php  echo 'data-target="#delete'.$show['exam_id'].'er"'; ?> > </span> Establish</span> </a>
                          </small></td>
        <?php }
                else if($show['examStatus']==3){
                  ?>
                          <td style="width: 138px;"> <small>
                            <a style="text-decoration:none;color:white;width: 48px; " class="btn btn-info btn-xs" href=""    data-toggle="modal"  <?php  echo 'data-target="#delete'.$show['exam_id'].'er"'; ?> > </span> Modify</span> </a>

                           | <a style="text-decoration:none;color:white; width: 70px;"  href="" class="btn btn-info btn-xs"   data-toggle="modal"  <?php  echo 'data-target="#confirmexam'.$show['exam_id'].'ers"'; ?> > </span><i class="fa fa-send"></i> Confirm Exam</span> </a></small></td>
                           </tr>
                          <?php
                      }
                     $count++;
...
<!--MODALS -->
 ...
 <div class="modal fade" <?php  echo 'id="delete'.$show['exam_id'].'er"';?>   tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                    ...
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span></button>
                      ...
                      <div class="modal-body">
                        ...
                        <form id="signup" method="post" action=""  class="well">
                                   ...
                                   <div class="row">
                <label class="col-md-3">Exam's Room </label>
                <select name="room"  required=""  class="col-md-9 form-control">
                  <option value="1" class="control" >FAC 1</option>
                   <option value="2" class="control" >FAC 2</option>
                   <option value="3" class="control" >LAB 1</option>
                   <option value="4" class="control" >LAB 2</option>
                   <option value="5" class="control" >READING ROOM</option>
                </select>
              </div><br>
   ...
   <?php } ?>
       ...
       <?php
 //select data from the database
                $select=mysqli_query($connect,"SELECT *,DATE_FORMAT(exam_date,'%d/%m/%Y') AS exam_d,exam.status as examStatus FROM exam inner join course on course.course_id=exam.course_id inner join class on class.class_id=exam.class_id inner join department on department.department_id=exam.department_id inner join test on exam.test=test.ID inner join user on user.user_id=course.lecture_id where  exam.status!=1 and exam.status!=4");
                        	while($show1=mysqli_fetch_array($select))
                        	{
                        	?>
 <div class="modal fade" <?php  echo 'id="confirmexam'.$show1['exam_id'].'ers"';?>   tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
                  ...
                  <h4 class="modal-title" id="myModalLabel">Admin Panel</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span></button>
                      ...
                      <div class="modal-body">
                        ...
                        <form id="signup" method="post" action=""  class="well">
            <div class="row">
                <label class="col-md-12">Confirm  this Exam : <strong><?php echo $show1['Name']. '</strong> of <strong>'.$show1['course_name'];?></strong> to take place !</label>
              </div>
                    ...
                    <center>          
                 <p><button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
                  <input  type="submit" style="background: rgb(58,155,235); color: white;" name="sendExam" value="Confirm this Exam" class="btn btn-info btn-sm" >
                     </p> </center>  
                         </form>  
                       </div>
                         <?php
