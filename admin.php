  ...
  <?php 
   ...
   if($SEEre['existuser']>=1){
      ...
      $enregistrer=mysqli_query($connect,"INSERT INTO user(names,email,phone,status,category_id,pin,password,c_date) values('$names','$email','$phone','$status','$category','$pin','$ps',NOW())");
        if($enregistrer) {
        echo '<script>
                                alert("Admin added successfully. Default Password : '.$ps.'")
                          </script>';
      }
      else{
        echo '<script>
...
<!--end of php codes -->
	...
	<div class="container-fluid " style="height: 50px; background: lightgrey"><br>
		<span class="text-right"><?php echo $view['names'];?> | <i style="color:grey;">Admin</i> </span>
	</div>
	...
	<section>
		<?php 
			...
			if($chkcateg['category_id']==1){
				?>
		<div class="col-lg-12">
			<button class="btn btn-info btn-xs col-lg-6" data-toggle="modal" data-target="#addnewuser"><i class="fa  fa-plus"> </i>  Add a new Admin</button>
		</div>
		<?php
			}
		?>
		<div class="col-lg-12">
              <div class="card">
                <div class="card-header">
                  <h2 style="font-family: serif;"><small>List od the Administrators</small></h2>
                </div>
                <div class="card-body">
		...
		?>
                      ...
                      <tbody>
                        	...
                        	$sel=mysqli_query($connect,"SELECT * FROM user where category_id=1");
...
<!--MODALS -->
 ...
 <div class="modal fade" <?php  echo 'id="delete'.$show['user_id'].'er"';?>   tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
   ...
   <?php } ?>
       ...
       <?php
 //select data from the database
                $select=mysqli_query($connect,"SELECT * FROM user where category_id=1");
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <div class="row">
              ...
              <div class="row">
                <select name="category"  hidden="" required=""  class="col-md-9 form-control">
                  <option value="1" class="control">Administrators</option>
                </select>
              </div>
               <div class="row">
                <label class="col-md-3">Status </label>
